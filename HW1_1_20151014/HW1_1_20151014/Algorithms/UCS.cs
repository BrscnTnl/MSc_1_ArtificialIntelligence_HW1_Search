        ...
        public List<BFSItem> Solve(int[,] Matrix, int sideLenght)
        {
            ...
            for (int i = 0; i < NodeCount; i++)
            {
                ...
                else
                {
                    ...
                    if (QueueList.Where(x => !x.isChecked).ToList().Count == 0 && GlobalLevel < 9)
                    {
                        GlobalLevel++;
                        foreach (BFSItem item in QueueList.Where(x => !x.isExpanded).ToList())
                        {
                            ...
                            QueueList.AddRange(BFSFabric(item.Matrix.GetNeighbourCombinationsForBFS(sideLenght), GlobalLevel, item.ID, sideLenght));
                            QueueList = QueueList.OrderBy(x => x.Cost).ToList();
                        }
                ...
                if (!isEqual)
                    QueueListTemp.Add(
                        new BFSItem()
                        {
                            ...
                            parentID = Parent,
                            Cost = _Matrix.GetCost(_sideLenght)
